---
name: update-readme
description: Update README.md so it stays true to the codebase — what the project is, how to run it, its commands/tools, configuration/env vars, and setup. Runs as a step in `raise-pr` (refresh the README before opening a PR) and on demand. Makes the smallest faithful edit, or none if nothing reader-facing changed.
---
...
Read the current `README.md`, then read what defines reality for **this repo's stack** — don't
write from memory. Detect the stack and go to its real anchors:

- **What it is / entry point / how to run it** — the main entry/CLI and the run scripts.
  - Node/TS: `package.json` (`scripts`, `bin`) and the entry under `src/`.
...
- **Install & dependencies** — `package.json` + lockfile, or `pyproject.toml`/`requirements*.txt`.
- **Tests** — the test config and directory (e.g. `vitest`/`jest` config, `pytest.ini`/`pyproject`).
