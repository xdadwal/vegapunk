...
Unguarded: it reads Vegapunk's own skills directory under ``.vegapunk/``, not
the user's workspace, so it doesn't go through the approval gate. The store
lives in ``vegapunk/skills.py``; the one-line ads in the system prompt tell
the model what exists, and this tool is how a matching skill's body actually
enters context — the on-demand half of progressive disclosure.
"""
...
def use_skill(name: str) -> str:
    ...
    description."""
    try:
        canonical, body = skills.load_skill(name)
        ...
        if not names:
            return (
                ...
                f"(The user can add skills as markdown files under {skills.skills_dir()}.)"
            )
        return (
    ...
    return f"Skill '{canonical}' loaded. Follow these instructions to do the task:\n\n{body}"
